...
async function closeDealWon(conn, { sellerId, propertyId }) {
  ...
  await conn.query("UPDATE sales_pipeline SET stage = 'Closed' WHERE property_id = ?", [propertyId]);
  ...
  return ` Deal closed. Property status set to "Complete", pipeline stage set to "Closed", removed ${delResult.affectedRows} task(s).${gpiNote}`;
}
...
async function closeDealLost(conn, { sellerId, propertyId }) {
  ...
  const [delResult] = await conn.query('DELETE FROM tasks WHERE property_id = ?', [propertyId]);
  const gpiNote = await cleanUpSellerAcquisitionTaskIfNoOtherActiveProperties(conn, sellerId, propertyId);
  return ` Deal closed. Property status set to "Cancelled", pipeline stage set to "Closed", removed ${delResult.affectedRows} task(s).${gpiNote}`;
}
