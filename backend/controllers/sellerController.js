...
const { closeDealLost, reopenDeal } = require('../utils/dealLifecycle');
...
//
// Status changed TO "Inactive"/"Invalid":
//   - Seller has NO property at all: removed from Task and Pipeline entirely (their
//     seller-level "Get Property Info" task and unattached pipeline entry are deleted).
//   - Seller HAS properties: any property still "On Process" is auto-closed as LOST
//     (status -> Cancelled, pipeline stage -> Closed - Lost, its tasks removed) via the
//     same closeDealLost() used elsewhere. Properties already "Complete" or "Cancelled"
//     are left completely untouched — they're historical, unrelated to this seller going
//     inactive.
//
// Status changed back TO "Active" (from Inactive/Invalid):
//   - Seller has NO property at all: a fresh "Get Property Info" task + "New Lead"
//     pipeline entry are recreated, exactly like a brand-new registration.
//   - Seller HAS properties: any property currently "Cancelled" is reopened via
//     reopenDeal() (status -> On Process, stage -> whatever TASK_FLOW says follows "Get
//     Property Info", e.g. "Qualify", with a fresh next task, e.g. "Review Property
//     Info"). Properties already "Complete" are left untouched — a won deal doesn't
//     un-close itself just because the seller is active again.
exports.updateSeller = async (req, res) => {
  ...
  try {
    ...
    if (becameInactiveOrInvalid) {
      const [propertiesForSeller] = await conn.query(
        'SELECT id, status FROM properties WHERE seller_id = ?',
        [id]
      );

      if (propertiesForSeller.length === 0) {
        // No property on file at all — removed from Task/Pipeline entirely.
        ...
        cleanupNote = ` Status set to "${finalStatus}": removed ${taskDelResult.affectedRows} task(s) and ${pipelineDelResult.affectedRows} pipeline entr${pipelineDelResult.affectedRows === 1 ? 'y' : 'ies'} (no property on file).`;
      } else {
        // Auto-close any still-active property as LOST; leave already-closed ones alone.
        let closedCount = 0;
        for (const property of propertiesForSeller) {
          if (property.status === 'On Process') {
            await closeDealLost(conn, { sellerId: id, propertyId: property.id });
            closedCount++;
          }
        }
        cleanupNote = closedCount > 0
          ? ` Status set to "${finalStatus}": ${closedCount} active propert${closedCount === 1 ? 'y was' : 'ies were'} auto-closed as Cancelled (Closed - Lost).`
          : ` Status set to "${finalStatus}": no active properties needed closing.`;
      }
    } else if (becameActiveAgain) {
      const [propertiesForSeller] = await conn.query(
        'SELECT id, status, employee_id FROM properties WHERE seller_id = ?',
        [id]
      );

      if (propertiesForSeller.length === 0) {
        // No property on file — restore exactly like a brand-new registration, guarding
        // against duplicates in case something was already left over.
        ...
        cleanupNote = notes.length > 0
          ? ` Reactivated: ${notes.join(' and ')} (no property on file).`
          : ' Reactivated: task and pipeline entry already existed, nothing duplicated.';
      } else {
        // Reopen any Cancelled property; leave Complete ones alone.
        let reopenedCount = 0;
        for (const property of propertiesForSeller) {
          if (property.status === 'Cancelled') {
            await reopenDeal(conn, { sellerId: id, propertyId: property.id, employeeId: property.employee_id });
            reopenedCount++;
          }
        }
        cleanupNote = reopenedCount > 0
          ? ` Reactivated: ${reopenedCount} cancelled propert${reopenedCount === 1 ? 'y was' : 'ies were'} reopened.`
          : ' Reactivated: no cancelled properties to reopen.';
      }
    }
