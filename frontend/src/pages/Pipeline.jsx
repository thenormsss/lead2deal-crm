...
// NOTE: "Qualify" (not "Qualified") — matches the stage name used in taskFlow.js and the
// sales_pipeline.stage enum in the database.
const STAGES = [
  { name: 'New Lead', color: '#6c9bd1' },
  { name: 'Contacted', color: '#4aa3c4' },
  { name: 'Appointment', color: '#f0a94e' },
  { name: 'Offer', color: '#f0da6e' },
  { name: 'Under Contract', color: '#b06fd6' },
  ...
  return (
    <div className="app-layout">
      ...
      <main className="main-content">
        <div className="panel">
          <h1 className="panel-title">SELLER STAGE</h1>
                ...
                <div className="pipeline-cards-lane">
                  {cards.map((row) => (
                    <div className="pipeline-card" style={{ borderLeftColor: stage.color }} key={row.id}>
                      ...
                      <div className={'pipeline-card-team' + (row.team ? '' : ' pipeline-card-placeholder')}>
                        {row.team ? `${row.team} — ${row.agent_name}` : ''}
                      </div>
                    </div>
                  ))}
                  {cards.length === 0 && (
...
//                       <div className="pipeline-card-phone">{row.seller_phone}</div>
//                       <div className={'pipeline-card-team' + (row.team ? '' : ' pipeline-card-placeholder')}>
//                         {row.team ? `${row.team} — ${row.agent_name}` : 'No agent assigned yet'}
//                       </div>
//                     </div>
//                   ))}
//                   {cards.length === 0 && (
