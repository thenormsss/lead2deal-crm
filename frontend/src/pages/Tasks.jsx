            ...
            <table className="data-table">
              ...
              <tbody>
                {tasks.map((t) => {
                  ...
                  // The checkbox is ALWAYS locked for "Get Property Info" — it only ever
                  // completes as a side effect of recording a property, never by manual
                  // toggle. Delete stays allowed until a property exists (unchanged).
                  const checkboxLocked = isGetPropertyInfo;
                  const deleteLocked = isGetPropertyInfo && hasPropertyRecord;

                  return (
                    <tr key={t.id}>
                      <td>
                        <span
                          className={'task-check' + (t.status === 'Done' ? ' done' : '') + (checkboxLocked ? ' locked' : '')}
                          onClick={() => !checkboxLocked && toggleDone(t)}
                          title={checkboxLocked ? 'Completed automatically once a property is recorded for this seller' : 'Toggle done'}
                        >
                          {t.status === 'Done' ? '✓' : ''}
                        </span>
                      </td>
